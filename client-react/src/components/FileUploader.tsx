...
const FileUploader = () => {
  ...
  //     debugger;
  //     if (file.type.toLowerCase() === 'application/pdf' || file.type.toLowerCase() === 'audio/mpeg') 
  //     {
  ...
  const handleUpload = async () => {
    if (!file) return;
    try {
      // שלב 1: קבלת Presigned URL וה-S3 Key מהשרת
      interface PresignedUrlResponse {
        url: string;
        s3Key: string; // הנתיב הייחודי של הקובץ ב-S3
      }
debugger;
      const response = await axios.get<PresignedUrlResponse>('https://localhost:7170/api/FileUpload/presigned-url', { 
        params: { fileName: file.name }
...
debugger;
      ...
      const s3Key = response.data.s3Key; // קבלת ה-S3 Key
...
debugger;
        // שלב 3: שליחת פרטי הקובץ לשרת כדי לשמור ב-DB
        await axios.post('https://localhost:7170/api/FileUpload/UploadTo-DB', {
           fileName: file.name,
            FileUrl: presignedUrl.split('?')[0], // שמירת הנתיב המלא ללא ה-Query Params
            UserId:user.id,
            s3Key: s3Key, // שמירת ה-S3 Key
        },
      );
...
//עיצוב ש ל gpt ויש בו איזה שהיא תקלה והוא לא עובד...
